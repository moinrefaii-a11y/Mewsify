...
class MelodyAudioHandler extends BaseAudioHandler with QueueHandler, SeekHandler {
  ...
  /// Overlay player used only during crossfades. Lazily created.
  ///
  /// `handleAudioSessionActivation: false` + `handleInterruptions:
  /// false` keep it from grabbing audio focus when it starts — without
  /// that, starting the overlay pauses the main player and you'd hear
  /// only the incoming track from its start instead of a real overlap.
  ...
  /// Stop playback when the user swipes the app off the recents view.
  @override
  ...
  /// Reorder a queue item (drag-and-drop in the queue sheet). Additive
  /// and safe — `ConcatenatingAudioSource.move` is the intended API for
  /// gapless reordering and doesn't interrupt the currently-playing
  /// item. We keep `_queue`, `_concat`, and `_currentIndex` in sync.
  Future<void> moveQueueItem(int oldIndex, int newIndex) async {
    ...
    // Mirror into the materialised concat window when both indices are
    // within it (so the live gapless playlist stays aligned).
    if (oldIndex < _concat.length && newIndex < _concat.length) {
      try {
        await _concat.move(oldIndex, newIndex);
      } catch (_) {}
    } else {
      // One side is outside the resolved window — drop the resolved
      // tail and let it re-resolve lazily from the new order.
      // (Cheap + safe: only affects not-yet-played upcoming items.)
    }

    // Keep the "currently playing" pointer on the same track.
